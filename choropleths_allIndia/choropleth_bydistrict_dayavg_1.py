...
def plot_map(geodf,da_plt,mainmap,c_cmap,params):
    ...
    """
    scheme = mc.UserDefined(geodf['data'], params.classif)
    bins = mc.UserDefined(geodf['data'],
                       bins=params.classif).bins
    #norm = Normalize(0, len(bins))
    norm = BoundaryNorm(params.classif, c_cmap.N, clip=False)
    
    
    gplt.choropleth(
    geodf, hue='data', projection=gcrs.PlateCarree(),
    edgecolor='#000000', linewidth=.2,
    cmap=c_cmap, norm=norm,
    legend=False,ax=mainmap)    
